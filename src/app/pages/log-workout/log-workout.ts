...
export class LogWorkout {
  ...
  readonly categories = computed(() => [...new Set(this.exercises().map(e => e.category))].sort());
    ...
    this.workoutExercises.update(list =>
      list.map(w =>
        w.exerciseId === exerciseId
          ? { ...w, sets: [...w.sets, { reps: 0, weightKg: 0 }] }
          : w
      )
    );
  }
  ...
  saveWorkout(): void {
    ...
    this.workoutService.addWorkout({ date: this.date, notes: this.notes, exercises });
